...
def main():
    ...
    for i, filename in tqdm(enumerate(files), total=len(files)):
        ...
        km_labels = km.predict(X_n)
        km_chars = dist_chars(X_dist, km_labels, w_noise=False)
        km_chars['k'] = k
        km_chars['m'] = m
        km_chars['n'] = n
        km_chars['name'] = name
        km_chars['text_type'] = text_type
        km_chars['emb_type'] = emb_type
        km_chars['train/test'] = set_type.lower()

        update_dict(new_d=km_chars, d=km_table)
        if ((i != 0) and (i % n_buffer == 0)) or (i == len(files) - 1):
            pd.DataFrame(km_table).to_csv(table_path, mode='a', index=False, header=False)
            clean_dict(km_table)
